...
EquipmentAliases = namedtuple(
    "EquipmentAliases", ["base_name", "base_version", "alias_ids"]
)
...
def handle_base_variant(all_items, variant_item, base_name, base_versions):
    ...
    base_variant = next(
        (
            x
            for x in all_items
            if x["name"] == base_name and x["version"] in base_versions
        ),
        None,
    )
    if base_variant:
        data.setdefault(
            base_variant["id"], EquipmentAliases(base_name, base_variant["version"], [])
        ).alias_ids.append(variant_item["id"])
...
def main():
    ...
    for item in all_items:
        ...
        sanguine_torva_match = re.match(
            r"^Sanguine t(orva (full helm|platebody|platelegs))$", item["name"]
        )
        decoration_kit_match = re.match(
            r"(.*)\((?:g|t|(h)\d|Arrav|Asgarnia|Dorgeshuun|Dragon|Fairy|Guthix|HAM|Horse|Jogre|Kandarin|Misthalin|Money|Saradomin|Skull|Varrock|Zamorak|or|cr|Hallowed|Trailblazer|Ithell|Iorwerth|Trahaearn|Cadarn|Crwys|Meilyr|Hefin|Amlodd|upgraded|light|dark|dusk|lit|deadman)\)$",
            item["name"],
            re.IGNORECASE,
        )
        magic_robe_kit_match = re.match(
            r"^(?:Dark|Light|Twisted) ((?:infinity|ancestral) .*)$", item["name"]
        )
        ...
        # Locked variants
            ...
            if decoration_kit_match:
                handle_base_variant(
                    all_items, item, decoration_kit_match.group(1).strip(), ["Normal"]
                )
            # Only locked
            else:
        ...
        # Cosmetic Slayer helmets
            ...
            handle_base_variant(
                all_items,
                item,
                "Slayer helmet%s" % (slayer_helm_match.group(1) or ""),
                ["", "Nightmare Zone"],
            )
        # Sanguine Torva
            ...
            handle_base_variant(
                all_items,
                item,
                "T%s" % (sanguine_torva_match.group(1) or ""),
                ["Restored"],
            )
        # Amulet of glory variants
        elif (
            (item["name"] == "Amulet of glory" and item["version"] != "Uncharged")
            or item["name"] == "Amulet of glory (t)"
            or item["name"] == "Amulet of eternal glory"
        ):
        ...
        # Decoration kit variants
            ...
            if item["version"] in [
                ...
                base_item_name.endswith(" helm")
                and decoration_kit_match.group(2) == "h"
            ):
                handle_base_variant(
                    all_items,
                    item,
                    base_item_name.replace(" helm", " full helm"),
                    [item["version"]],
                )
            elif (
                base_item_name.endswith(" shield")
                and decoration_kit_match.group(2) == "h"
            ):
                handle_base_variant(
                    all_items,
                    item,
                    base_item_name.replace(" shield", " kiteshield"),
                    [item["version"]],
                )
            else:
        ...
        # Magic robe variants
            ...
            handle_base_variant(
                all_items, item, magic_robe_kit_match.group(1).capitalize(), [""]
            )
        ...
        # Moons armours
            ...
            re.match(r"^(Used|New)", item["version"]) and "Crystal" not in item["name"]
        ):
            handle_base_variant(all_items, item, item["name"], ["New"])
        # Dark Bow variants
        elif (item["name"] == "Dark bow" and item["version"] != "Regular") or item[
            "name"
        ] == "Dark bow (deadman)":
            handle_base_variant(all_items, item, "Dark bow", ["Regular"])
        # Granite maul variants
        elif (item["name"] == "Granite maul" and item["version"] != "Normal") or item[
            "name"
        ] == "Granite maul (or)":
        ...
        # Radiant oathplate variants
            ...
            handle_base_variant(
                all_items, item, item["name"].replace("Radiant ", "").capitalize(), ""
            )
    ...
    for k, v in sorted(data.items(), key=lambda item: item[1].base_name):
        dataJs += "\n  %s: %s, // %s%s" % (
            ...
            v.alias_ids,
            v.base_name,
            f"#{v.base_version}" if v.base_version else "",
        )
